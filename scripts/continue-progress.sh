...
with status_log.open(newline="") as file:
    for row in csv.DictReader(file, delimiter="\t"):
        ...
        if path.name in excluded_names or path.stem in excluded_stems:
            print(f"warning: excluded by rules, skipped in-progress image: {image}", file=sys.stderr)
        ...
        if (raw_dir / image).is_file():
            ...
            print(f"warning: raw image missing or not a file, skipped: {raw_dir / image}", file=sys.stderr)
PY
)
...
total="${#images[@]}"
if [ "$total" -eq 0 ]; then
  echo "No in-progress images to continue"
  exit 0
fi

echo "Continue in-progress watermark removal"
echo "Images: $total"
